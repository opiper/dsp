...
class WarehouseEnv(gym.Env):
    def __init__(self, width, height, cell_size, item_sizes_queue):
        ...
        self.predefined_order = [(r, c) for r in range(height) for c in range(width)]
        self.next_placement_index = 0
    ...
    def step(self, action):
        ...
        if self.next_placement_index >= len(self.predefined_order):
            # Handle the case where you've run out of predefined positions
            # This could be an immediate episode termination or a specific penalty
            return self.state, -10, True, {}

        predefined_row, predefined_col = self.predefined_order[self.next_placement_index]

        if (row, col) == (predefined_row, predefined_col) and self.state[row, col] >= item_size:
            # Correct placement according to the predefined order
            self.state[row, col] -= item_size
            self.item_sizes_queue.pop(0)
            self.placement_record.append((row, col, item_size))
            reward = 1  # Reward for correct placement
            self.next_placement_index += 1
            if not self.item_sizes_queue:  # Check if all items have been placed
                done = True
                reward += 50  # Large bonus for completing the task
        else:
            # Incorrect placement or the cell cannot accommodate the item
            reward = -1
            done = True  # Optionally, end the episode on incorrect placement
    ...
    def render(self, mode='human'):
        if self.unplaced_items:
            print(f"Unplaced items: {self.unplaced_items}")
...
print("Placement Record: ")
for placement in gym_env.placement_record:
    print(f"Item of size {placement[2]} placed at ({placement[0]}, {placement[1]})")
count = 0
